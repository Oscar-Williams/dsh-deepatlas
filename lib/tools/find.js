...
import { CAPABILITY_PARAMETER_SCHEMA, normalizeCapabilityIds } from '../core/capabilities.js';
export function buildFindTool(_ctx, config) {
    return {
        ...
        parameters: {
            need: { type: 'string', required: true, description: '任务需求原文' },
        ...
        async execute(args) {
            ...
            if (!status.exists) {
                return asLosslessJson({ ok: false, message: '本地索引不存在,请先调用 deepatlas_scan 重建索引' });
            }
            if (status.stale) {
                return asLosslessJson({ ok: false, message: '索引已过期,请先调用 deepatlas_scan 刷新' });
            }
            const index = await scanner.loadIndex();
            // Eligibility(⑦.0-d):死链/归档过滤 + 实体分类(框架/清单不可装)
            const plugins = (index?.plugins ?? []).filter((p) => p.stars >= config.minStars && !p.deadLink && p.kind !== 'framework' && p.kind !== 'collection');
