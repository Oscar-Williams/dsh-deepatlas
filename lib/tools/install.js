...
import { newPlan, approve, checkDuplicate, install, verifyComposed } from '../core/installplan.js';
...
export function buildInstallTool(_ctx, config) {
    return {
        ...
        parameters: {
            ...
            commit: { type: 'string', required: true, description: '锁定的 commit 短哈希(供应链安全,必填)' },
            auditLevel: { type: 'string', required: true, description: '最近一次 deepatlas_audit 的等级:green/yellow/red' },
            userConsent: { type: 'boolean', required: true, description: '用户是否明确同意安装,必须由用户亲口/显式操作给出' },
            enginesNode: { type: 'string', description: '目标包 engines.node(缺省按 unknown 评估)' },
        },
