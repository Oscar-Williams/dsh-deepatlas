...
async function fetchRepositoryFile(target, file, commit, token, signal) {
    ...
    try {
        ...
        if (!res.ok)
            return { text: null, error: `${file} 获取失败:GitHub API ${res.status}` };
...
async function fetchManifest(target, commit, token, signal) {
    const fetched = await fetchRepositoryFile(target, 'package.json', commit, token, signal);
    if (!fetched.text)
        return { manifest: null, error: fetched.error };
    try {
        return { manifest: JSON.parse(fetched.text) };
    }
    catch (error) {
...
function declaredSourceFiles(manifest) {
    const files = [];
    ...
    if (unsafe)
        return { files: normalized, error: `manifest 声明了非法仓库内路径:${unsafe}` };
...
export function buildAuditTool(_ctx, config) {
    return {
        ...
        parameters: {
            ...
            commit: { type: 'string', description: '锁定的完整 40 位 commit SHA;不传则只审计漂移的 HEAD' },
        },
        ...
        async execute(args, execution) {
            ...
            if (args.commit) {
                ...
                if (cached)
                    return asLosslessJson({ ...cached, action: '(来自审计缓存,同 commit 复用)' });
            }
