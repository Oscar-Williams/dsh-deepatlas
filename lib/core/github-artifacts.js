...
export async function listRepositoryRootAtCommit(repository, commit, token, signal, fetcher = fetch) {
    if (!isGithubRepoSlug(repository) || !isFullCommitSha(commit))
        throw new Error('根目录读取需要合法仓库与完整 commit SHA');
    const response = await fetcher(`${API}/repos/${repository}/contents?ref=${encodeURIComponent(commit)}`, {
        headers: headersFor(token), signal,
    });
    if (!response.ok)
        throw new Error(`根目录获取失败:GitHub API ${response.status}`);
    const body = await response.json();
    if (!Array.isArray(body))
        throw new Error('根目录获取失败:响应格式错误');
    return body
        .filter((entry) => typeof entry.name === 'string' && typeof entry.path === 'string' && (entry.type === 'file' || entry.type === 'dir'))
