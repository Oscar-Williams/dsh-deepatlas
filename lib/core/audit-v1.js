...
export function scanSourceSignals(files) {
    const out = [];
    for (const [name, content] of Object.entries(files)) {
        for (const p of PATTERNS) {
            if (p.re.test(content))
                out.push({ signal: p.signal, evidence: `${name} 命中 ${p.signal}`, level: p.level });
        }
    }
    return out;
}
export function buildAuditReportV1(input, files = {}) {
    ...
    const opaque = Object.entries(deps)
        ...
        .map(([k]) => k);
    const reasons = base.findings.map((f) => `${f.rule}:${f.evidence}`);
    ...
    const riskLevel = base.level === 'red' ? 'red' : elevated.length ? 'elevated' : base.level;
    return {
        ...base,
...
//# sourceMappingURL=audit-v1.js.map