...
function atomWeight(atom, state) {
    ...
    if (atom.signal.kind === 'manifest-declaration')
        return atom.provenance.authority === 'publisher' ? 0.95 : 0.4;
    if (atom.signal.kind === 'implementation')
        return atom.provenance.authority === 'publisher' ? 0.9 : 0.4;
    if (atom.signal.kind === 'exact-topic') {
        if (atom.provenance.authority === 'platform')
            return 0.55;
        if (atom.provenance.authority === 'community')
            return 0.4;
        return 0.8;
    }
    if (atom.signal.kind === 'publisher-text') {
        if (atom.provenance.authority === 'platform')
            return atom.provenance.jsonPointer === '/name' ? 0.35 : 0.55;
        if (atom.provenance.authority === 'community')
            return 0.4;
