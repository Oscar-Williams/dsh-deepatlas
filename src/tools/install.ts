...
import { newPlan, approve, checkDuplicate, install, verifyComposed, markFailed, rollbackToSnapshot, InstallPlan } from '../core/installplan.js'
...
import { discardSnapshot, restoreProfile, snapshotProfile, type ProfileSnapshot } from '../core/rollback.js'
import { checkCompatibility, getRuntimeInfo } from '../core/compat.js'
import { toRequirement } from '../core/record.js'

const exec = promisify(execFile)

async function run(cmd: string, args: string[], signal?: AbortSignal): Promise<{ code: number; output: string }> {
  try {
...
async function runDsh(args: string[], signal?: AbortSignal): Promise<{ code: number; output: string }> {
...
export function buildInstallTool(_ctx: Context, config: DeepAtlasConfig) {
  return {
    ...
    parameters: {
      ...
      commit: { type: 'string' as const, required: true, description: '锁定的完整 40 位 commit SHA(供应链安全,必填)' },
    ...
    async execute(args: {
      ...
      commit: string
      userConsent: boolean
    }, execution?: ToolExecutionContext): Promise<{ ok: boolean; dryRun?: boolean; executed?: boolean; composed?: boolean; active?: boolean; plan: InstallPlan; error?: string }> {
      ...
      if (plan.state === 'APPROVED') {
        plan = await install(plan, config.dryRun, async () =>
          runDsh(['plugin', '--profile', config.installProfile, 'add', `github:${target}#${args.commit}`], execution?.signal))
      }
      ...
      if (plan.state === 'INSTALLED' && !config.dryRun) {
        ...
        if (plan.state === 'INSTALLED') {
          ...
          if (snapshot) plan = await rollbackToSnapshot(plan, restoreSnapshot)
        }
      }
      if (plan.state === 'COMPOSED' && snapshot) await discardSnapshot(snapshot)
