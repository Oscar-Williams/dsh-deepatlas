...
function atomFor(capabilityId: string, part: CapabilityTextPart, alias: string): EvidenceAtom | null {
  const match = aliasPattern(alias).exec(part.text.toLowerCase())
  if (!match) return null
  const hit = match[2] ?? match[1] ?? alias
  const hitAt = match.index + match[0].indexOf(hit)
  ...
  const identity = { capabilityId, signal, provenance, extractor: EVIDENCE_EXTRACTOR_VERSION }
  return {
    evidenceId: stableHash(identity),
    subject: `capability:${capabilityId}`,
    polarity: 'supports',
    signal,
    provenance: { ...provenance, contentSha256: provenance.contentSha256 ?? stableHash(part.text) },
...
export function extractCapabilityEvidence(parts: CapabilityTextPart[], state: PluginEvidence['state'] = 'complete'): PluginEvidence {
  ...
  const uniqueAtoms = [...new Map(atoms.map((atom) => [atom.evidenceId, atom])).values()]
...
export function computeCapabilityClaims(atoms: EvidenceAtom[], state: PluginEvidence['state']): CapabilityClaim[] {
  ...
  return [...grouped.entries()].map(([id, evidence]) => {
    ...
    const strongestByOrigin = new Map<string, number>()
    for (const atom of support) {
      const weight = atomWeight(atom, state)
      strongestByOrigin.set(atom.provenance.originGroup, Math.max(strongestByOrigin.get(atom.provenance.originGroup) ?? 0, weight))
    }
    const sorted = [...strongestByOrigin.values()].sort((a, b) => b - a)
    ...
    const decision: CapabilityClaim['decision'] = contradiction.length && strongestContradiction >= strongest
      ? 'conflicted'
