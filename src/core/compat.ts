    ...
    .some((part) => {
      ...
      switch (op) {
        ...
        case '^':
          ...
          if (rMinor === undefined) return true
          if (minor !== rMinor) return minor > rMinor
          return rPatch === undefined || patch >= rPatch
        case '~':
          if (major !== rMajor || rMinor === undefined || minor !== rMinor) return major === rMajor && minor === rMinor
          return rPatch === undefined || patch >= rPatch
        case '>=':
...
export interface CompatibilityResult {
  ...
  ok: boolean
  node: { pass: boolean | 'unknown'; detail: string }
  platform: { note: string }
  build: { required: boolean; note: string }
  reasons: string[]
}
...
export function checkCompatibility(req: CompatibilityRequirement, runtime: RuntimeInfo): CompatibilityResult {
  const reasons: string[] = []

  let node: CompatibilityResult['node']
  if (!req.enginesNode) {
    ...
    if (!pass) reasons.push(node.detail)
  }
