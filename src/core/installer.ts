...
export function buildCommand(req: InstallRequest): string {
...
/** 闸门检查:同意与风险等级都满足才放行 */
export function planInstall(req: InstallRequest, dryRun: boolean): InstallPlan {
 ...
 */
export async function executeInstall(plan: InstallPlan, _exec?: (cmd: string) => Promise<string>): Promise<string> {
  if (!plan.allowed) throw new Error(plan.blockedReason)
